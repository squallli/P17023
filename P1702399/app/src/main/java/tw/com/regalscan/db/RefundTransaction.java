...
public class RefundTransaction {
    ...
    //交易金額資訊
    public double USDTotalAmount;    //美金應找零總金額
     ...
     * 初始化Transaction
     */
    public RefundTransaction(Context context, String SecSeq) {
    ...
    public JSONObject GetBasketInfo(String ReceiptNo) {
        ...
        String ClassName = "RefundTransaction";
        ...
        try {
            ...
            //檢核單據號碼是否允許進行DFS退貨
            SQL = "SELECT SalesHead.TotalPrice, SalesHead.ReceiptNo, SalesHead.IFEOrderNo, SalesHead.AuthuticationFlag, Inventory.DrawNo, SerialCode, " +
                    "SalesDetail.ItemCode, AllParts.ItemName, SalesDetail.OriPrice, SalesDetail.SalesQty " +
                    "FROM SalesHead LEFT JOIN SalesDetail " +
                    "ON SalesHead.ReceiptNo = SalesDetail.ReceiptNo " +
                    "LEFT JOIN Inventory " +
                    "ON SalesDetail.[ItemCode] = Inventory.[ItemCode] " +
                    "LEFT JOIN AllParts " +
                    "ON SalesDetail.[ItemCode] = AllParts.[ItemCode] " +
                    "WHERE Inventory.Secseq = '" + this.SecSeq +
            ...
            this.IFEOrderNo = jaItem.getJSONObject(0).getString("IFEOrderNo");
            ...
            for (int i = 0; i < jaItem.length(); i++) {
                ...
                _DFSItem.OriginalPrice = jaItem.getJSONObject(i).getDouble("OriPrice");
            ...
            for (int i = 0; i < jaItem.length(); i++) {
                ...
                if (!jaItem.getJSONObject(i).getString("PayBy").equals("Change") && jaItem.getJSONObject(i).getString("Currency").equals("TWD")) {
    ...
    public JSONObject AddRefundList(String Currency, double Amount) {
        ...
        String ClassName = "RefundTransaction";
        String FunctionName = "AddRefundList";
        String ReturnCode, ReturnMessage;
        TSQL _TSQL = TSQL.getINSTANCE(context, this.SecSeq, "P17023");
        JSONObject TempJo;
        ...
        try {
            ...
            //取得該金額所代表美金金額
            if ((Currency.equals(LastChangeCurrency) && Amount == LastChangeAmount) || (Amount == MaxChangeAmount)) {
                //直接帶美金最後應找零額
...
//            if (Arith.add(USDTotalChanged,USDAmount) > USDTotalAmount) {
            ...
            for (int i = 0; i < PaymentList.size(); i++) {
                if (PaymentList.get(i).PayBy.equals("Cash") && PaymentList.get(i).Currency.equals(Currency)) {
...
//        _TSQL.WriteLog(this.SecSeq, "System", ClassName, FunctionName, "Function start");
    ...
    public JSONObject GetCurrencyMaxAmount(String Currency) {
        ...
        try {
            ...
            //若全部都以台幣付款，則以台幣計價
            if ((IsTotalUseTWDPay && IsTotalUseTWDRefund && Currency.equals("TWD") || (IsTotalUseTWDPay && PaymentList.size() == 0 && Currency.equals("TWD")))) {
        ...
        try {
            ...
            for (int i = 0; i < DFSItemList.size(); i++) {
                ...
                SQL = "UPDATE Inventory " +
                        "SET SalesQty = SalesQty - " + DFSItemList.get(i).SalesQty +
                        ", EndQty = EndQty + " + DFSItemList.get(i).SalesQty +
                        " WHERE SecSeq = '" + this.SecSeq +
...
//                _TSQL.WriteLog(this.SecSeq, "System", ClassName, FunctionName, "Function start");
    ...
    public String getIFEOrderNO() {
        return this.IFEOrderNo;
    }
    ////////////////////////////////////////////////////////////////////////
    //                        public functions
    ////////////////////////////////////////////////////////////////////////
